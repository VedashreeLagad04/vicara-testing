...
class NotificationAndFallHistory {
  ...
  Future<dynamic> getNotifications({int pageSize = 3, int pageNumber = 0}) async {
    ...
    try {
      apiResponse = await Dio().get(
        apiBaseUrl +
            'notificationAndHistory/getNotifications?pageNo=$pageNumber&pageSize=$pageSize',
        options: Options(
            headers: {'Content-Type': 'application/json', 'Authorization': 'Bearer $token'}),
      );
    } on Exception catch (err, stacktrace) {
      await FirebaseCrashlytics.instance
          .recordError(err, stacktrace, reason: "while calling get notifications API");
      ...
      throw "error while making get notification request to server";
    }
  ...
  Future<dynamic> getFallHistory({int pageSize = 5, int pageNumber = 0}) async {
    ...
    try {
      ...
      await FirebaseCrashlytics.instance
          .recordError(err, stacktrace, reason: "while calling get fall history API");
      throw "error while making get fall history request to server";
    }
    if (apiResponse.statusCode == 200) {
      return apiResponse.data;
    } else {
