...
class ServerStatus extends StatefulWidget {
  ...
  const ServerStatus({required this.notificationSink, Key? key})
      : super(key: key);
...
class _ServerStatusState extends State<ServerStatus> {
  ...
  final StreamController<bool> _locationConnectionStream =
  ...
  final StreamController<bool> _wsConnectionStream =
        ...
        (event) async {
          ...
          if (result['type'] == 'Fall') {
            try {
              ...
              await _emergencySMSServiceApi.sendEmergencyMessage(lat: location.latitude, long: location.latitude);
              ...
              await FirebaseCrashlytics.instance.recordError(err, stacktrace,
                  reason: "showing event notification");
              Fluttertoast.showToast(
                msg: err.toString(),
                toastLength: Toast.LENGTH_SHORT,
                gravity: ToastGravity.CENTER,
